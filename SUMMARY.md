
# Java体系学习

# Linux基础

- [Linux](knowledge/Linux/README.md)
    - [Linux虚拟机](knowledge/Linux/笔记2-Linux虚拟机.md)
    - [X-Shell命令](knowledge/Linux/笔记3-X-shell.md)
    - [Linux文件系统相关的命令](knowledge/Linux/笔记4-Linux-文件系统相关的命令.md)
    - [正则表达式](knowledge/Linux/笔记5-正则表达式.md)
    - [文本数据处理命令](knowledge/Linux/笔记6-文本数据处理命令.md)
    - [Linux软件安装](knowledge/Linux/笔记7-Linux软件安装.md)

# Java知识
- [JAVA架构师之路](knowledge/JAVA架构师之路.md)
- [Java知识](knowledge/README.md)
- [J2SE基础](knowledge/java/basic/README.md)
    - [Java集合](knowledge/java/basic/Java集合.md)
        - [Java队列](knowledge/java/basic/Java队列.md)
        - [Java栈](knowledge/java/basic/Java栈.md)
        - [PriorityQueue](knowledge/java/basic/PriorityQueue.md)
        - [List和Array用法](knowledge/java/basic/List和Array用法.md)
    - [Java工具类](knowledge/java/utils/README.md)
        - [BeanUtils](knowledge/java/utils/BeanUtils.md)
        - [Date类](knowledge/java/utils/Date.md)
        - [常见加密算法](knowledge/java/utils/encryption.md)
        - [RSA数字证书生成](knowledge/java/utils/RSA数字证书生成.md)
    - [Java的classpath指什么](knowledge/java/Java的classpath指什么.md)
    - [java和javaw的区别](knowledge/java/java和javaw的区别.md)
- [Spring](knowledge/java/spring/README.md)
  - [spring框架](knowledge/java/spring/springframework/README.md)
    - [Spring常用注解总结](knowledge/java/spring/springframework/Spring常用注解总结.md)
    - [springtest](knowledge/java/springtest.md)
  - [SpringBoot](knowledge/java/spring/springboot/README.md)
    - [springboot同时支持http和https访问](knowledge/java/spring/springboot/springboot同时支持http和https访问.md)
    - [SpringBoot使用腾讯云SSL证书并开启HTTPS访问](knowledge/java/spring/springboot/SpringBoot使用腾讯云SSL证书并开启HTTPS访问.md)
  - [SpringCloud](knowledge/java/spring/springcloud/README.md)
- [框架工具](knowledge/框架工具/README.md)
    - [logback](knowledge/框架工具/logback/README.md)
    - [Maven](knowledge/框架工具/maven/mavencmd.md)
        - [pom介绍](knowledge/框架工具/maven/maven_pom.md)
    - [Mybatis](knowledge/java/mybatis.md)
    - [Tomcat](knowledge/tomcat.md)
    - [线程池](knowledge/java/threadpool.md)
    - [Lambda表达式](knowledge/java/java8/Lambda.md)
- [JVM](knowledge/java/jvm/jvm.md)
    - [JVM基础工具](knowledge/java/jvm/jvmtools_1_基础工具.md)
    - [JVM: arthas](knowledge/java/jvm/jvmtools_2_arthas.md)
    - [Arthas活用ognl表达式](knowledge/java/jvm/jvmtools_3_ognl.md)
    - [JVM1:内存组成](knowledge/java/jvm/jvm1.md)
    - [JVM2:HotSpot垃圾收集器的种类](knowledge/java/jvm/jvm2.md)
    - [JVM3:GC策略&内存申请、对象衰老](knowledge/java/jvm/jvm3.md)
    - [JVM4:JVM参数优化设置](knowledge/java/jvm/jvm4.md)
    - [JVM5:生产环境实例分析](knowledge/java/jvm5.md)
    - [JVM6:JVM监测工具](knowledge/java/jvm/jvm6.md)
    - [JVM7:classLoader的卸载机制](knowledge/java/jvm/jvm7.md)
    - [JVM8:Dynamic Code Evolution for Java dcevm原理](knowledge/java/jvm/jvm8.md)
    - [一文看懂JVM的GC](knowledge/java/jvm/一文看懂JVM的GC.md)  

# 算法

- [基本数据结构与算法](src/main/dev/algorithm/README.md)
    - [五大常用算法](src/main/dev/algorithm/五大常用算法.md)
- [leetcode](src/dev/leetcode/README.md)

# 其他

- [小技巧](knowledge/java/basic/tips.md)
- [原型开发](knowledge/prototype.md)
        
  
# 推荐

- [书单推荐](BookList.md)